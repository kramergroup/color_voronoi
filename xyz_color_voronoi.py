...
def color_voronoi(vor, z, ax,
                  ...
                  p_size=1,
                  show_points=True, show_vertices=False,
    ...
    for i, p_r in enumerate(vor.point_region):
        ...
        if not -1 in region:
            ...
            ax.fill(*zip(*polygon),
                    alpha=pol_alpha, linewidth=0, # No edges, voronoi_plot_2d takes care of that
...
def xyz_color_vornoi(argv):
    ...
    - p_size=0.5 (point size),
    - show_points=True (write 0/1 in json, not string),
    - show_vertices=False (0/1),
    - l_width=1 (diagram line width),
    - l_color='red' (diagram lince color),
    - l_alpha=1 (diagram line alpha, 0 to hide construction)
    - pol_alpha=1 (polygon color alpha)
    """
    ...
    parser.add_argument('-o', '--out',
                        dest='out_f',
                        type=str, default=None, metavar="basename.ext",
                        help='save figure to file. Estension inferred as last bit after last ".";')
    ...
    # Output file details
    dpi = args.dpi
    # Infer estension from filename: split according to dots and take last part
    #if args.out_f is not None:
    #    out_format = args.out_f.split(".")[-1]
    #    c_log.debug("Output format set to %s", out_format)
    # Matplotlib is perfectly capable of understanding it on its own
    ...
    # First, save figure
    if args.out_f != None:
        c_log.info("Saving file on %s", args.out_f)
        plt.savefig(args.out_f, dpi=dpi)
